...
import { fetchAdminCountsForPermissions, fetchAdminOverviewForPermissions } from "@/lib/adminOverviewData";
...
export function AdminContent() {
  ...
  const canViewAnalytics = can("admin.analytics");
  const allowedPermissions = PERMISSIONS.filter(permission => can(permission.key)).map(permission => permission.key);
  const accessKey = allowedPermissions.join(",");
  const hasTools = allowedPermissions.some(permission => ["admin.competitions", "admin.workshops", "admin.problems", "admin.users", "admin.updates", "admin.badges", "admin.shop", "admin.contact", "admin.announcements", "admin.email", "admin.lessons", "admin.design-system", "admin.daily", "admin.classes", "admin.moderation"].includes(permission));
  const queryClient = useQueryClient();

  const countsQuery = useQuery({
    queryKey: ["admin", "counts", user?.id, accessKey],
    queryFn: () => fetchAdminCountsForPermissions(allowedPermissions),
  });

  const overviewQuery = useQuery({
    queryKey: ["admin", "overview", user?.id, accessKey],
    queryFn: () => fetchAdminOverviewForPermissions(allowedPermissions),
    enabled: canViewAnalytics,
  });

  const onboardingQuery = useQuery({
    queryKey: ["admin", "onboarding-stats", user?.id, accessKey],
    enabled: canViewAnalytics,
  ...
  const analyticsQuery = useQuery({
    queryKey: ["admin", "analytics", days, user?.id, accessKey],
    enabled: canViewAnalytics,
  ...
  function snapshot() {
    return JSON.stringify([
      queryClient.getQueryData(["admin", "counts", user?.id, accessKey]) ?? null,
      queryClient.getQueryData(["admin", "overview", user?.id, accessKey]) ?? null,
      queryClient.getQueryData(["admin", "analytics", days, user?.id, accessKey]) ?? null,
      queryClient.getQueryData(["admin", "tasks"]) ?? null,
      queryClient.getQueryData(["admin", "onboarding-stats", user?.id, accessKey]) ?? null,
    ]);
  }
  ...
  async function refreshAll() {
    ...
    try {
      await Promise.all([
        ...
        ...(canViewAnalytics ? [overviewQuery.refetch({ throwOnError: true })] : []),
        ...(canViewAnalytics ? [analyticsQuery.refetch({ throwOnError: true })] : []),
        ...(canViewAnalytics ? [onboardingQuery.refetch({ throwOnError: true })] : []),
        ...(can("admin.tasks") ? [queryClient.invalidateQueries({ queryKey: ["admin", "tasks"] })] : []),
      ]);
    } catch {
    ...
    <div className="space-y-6">
      ...
      >
        <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,14rem),1fr))] items-stretch gap-4">
          {can("admin.competitions") && <div className="min-w-0">
            <AdminNavCard
              ...
              description={locale === "ro"
                ? "Configurează concursuri, probleme și invitații."
                : "Configure competitions, problems, and invites."}
              count={null}
            />
          </div>}

          {can("admin.workshops") && <div className="min-w-0">
            <AdminNavCard
          ...
          {(can("admin.problems") || can("admin.daily")) && <div className="min-w-0">
            <AdminNavCard
              href="/admin/problems"
              icon={<FileText className="h-5 w-5 text-blue-500" />}
              ringClassName="ring-blue-500/50"
              title={t("admin.problems.title")}
              description={can("admin.problems") ? t("admin.problems.description") : locale === "ro" ? "Programează problemele zilei." : "Schedule daily problems."}
              count={counts?.problems}
            />
          </div>}

          {can("admin.users") && <div className="min-w-0">
            <AdminNavCard
              href="/admin/users"
          ...
          {can("admin.updates") && <div className="min-w-0">
            <AdminNavCard
              href="/admin/updates"
          ...
          {can("admin.badges") && <div className="min-w-0">
            <AdminNavCard
              href="/admin/badges"
          ...
          {can("admin.shop") && <div className="min-w-0">
            <AdminNavCard
              href="/admin/shop"
          ...
          {can("admin.contact") && <div className="min-w-0">
            <AdminNavCard
              href="/admin/contact"
          ...
          {can("admin.announcements") && <div className="min-w-0">
            <AdminNavCard
              href="/admin/announcements"
              icon={<Megaphone className="h-5 w-5 text-amber-500" />}
              ringClassName="ring-amber-500/50"
              title={locale === "ro" ? "Anunțuri" : "Announcements"}
              description={locale === "ro"
                ? "Trimite anunțuri în notificările utilizatorilor platformei."
                : "Send announcements to platform users’ notifications."}
              count={null}
            />
          </div>}

          {can("admin.email") && <div className="min-w-0">
            <AdminNavCard
              href="/admin/email"
          ...
          {can("admin.lessons") && <div className="min-w-0">
            <AdminNavCard
              href="/admin/lessons"
          ...
          {can("admin.design-system") && <div className="min-w-0">
            <AdminNavCard
          ...
          {can("admin.classes") && <AdminNavCard href="/classes" icon={<GraduationCap className="h-5 w-5" />} ringClassName="ring-foreground/20" title={locale === "ro" ? "Clase" : "Classes"} description={locale === "ro" ? "Administrează clasele și temele." : "Manage classes and assignments."} count={null} />}
          {can("admin.moderation") && <AdminNavCard href="/feed" icon={<Users className="h-5 w-5" />} ringClassName="ring-foreground/20" title={locale === "ro" ? "Moderare" : "Moderation"} description={locale === "ro" ? "Gestionează postările comunității." : "Manage community posts."} count={null} />}
        </div>
      </DashboardSection>
      </>}
      ...
      {canViewAnalytics && <AdminPanelSection
